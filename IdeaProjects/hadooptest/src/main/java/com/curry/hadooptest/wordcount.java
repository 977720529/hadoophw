...
//                hasMoreTokens() 方法是用来测试是否有此标记生成器的字符串可用更多的标记。
    ...
    public static void main(String[] args) throws Exception {
        ...
        //传到集群上，然后通过命令来执行这个作业，但是命令中不必指定JAR文件的名               称，在这条命令中通过job对象的setJarByClass（）中传递一个主类就行，hadoop会通过这个主类来查找包含它的JAR文件。
...
//                                这句话要好好理解!!!
        ...
        FileOutputFormat.setOutputPath(job, new Path(
                "hdfs://hserver1:9000/hdfsOutput"));//只能有一个输出路径，该路径指定的就是reduce函数输出文件的写入目录。
